...
- `ssr: true` is the default Nuxt 3 behaviour and is required for SSG.
- There is no `target: 'static'` option in Nuxt 3. SSG is achieved solely through the `nuxt generate` command.
...
- Frontend (Nuxt 3) and CMS (Strapi) are fully decoupled.
...
- **Single-page scroll** homepage (`pages/index.vue`) containing all sections:
...
- All Strapi data is fetched in `pages/index.vue` using parallel requests.
...
|------------------------|-----------------------------------------|---------------|--------------|
| `components/ui/`       | Stateless, reusable, design system      | No            | No           |
| `components/layout/`   | App shell (header, footer, nav)         | No            | No           |
| `components/sections/` | Page sections, receive data via props   | No            | Yes (types)  |
| `components/content/`  | Content display (cards, timeline items) | No            | Yes (types)  |
| `components/animations/` | Visual effects, particles, scroll     | No            | No           |
| `pages/`               | Data fetching, SEO, section composition | Yes           | Yes          |
